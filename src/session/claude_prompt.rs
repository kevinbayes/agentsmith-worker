...
/// A Claude Code session that runs `claude -p "<text>"` per message.
///
/// The first message creates a new session. Subsequent messages use
/// `--continue` to resume the most recent conversation.
///
/// No PTY, no ANSI stripping, no interaction agent — just clean text output.
pub struct ClaudePromptSession {
    config: ClaudeConfig,
    working_dir: PathBuf,
...
impl ClaudePromptSession {
    ...
    pub async fn start(&mut self) -> Result<()> {
        tracing::info!("Claude prompt session ready");
        Ok(())
    }

    /// Send user input by spawning a `claude -p` process.
    pub async fn send_input(&mut self, text: &str) -> Result<()> {
        {
            let st = self.state.lock().await;
            if *st == PromptState::Running {
                anyhow::bail!("Claude is still processing the previous message. Please wait.");
            }
        }

        // Build command arguments
        // First message: plain `claude -p` (creates a new session).
        // Subsequent messages: `claude -p --continue` (resumes the most recent session).
        let mut args = vec![
            "-p".to_string(),
            text.to_string(),
        ];
...
impl Drop for ClaudePromptSession {
    fn drop(&mut self) {
