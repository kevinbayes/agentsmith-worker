...
/// A Goose session that runs `goose run -n <name> -t "text"` per message.
///
/// The first message creates a named session. Subsequent messages use
/// `-r` to resume the named session.
///
/// No PTY, no ANSI stripping, no interaction agent — just clean text output.
pub struct GoosePromptSession {
    config: GooseConfig,
    working_dir: PathBuf,
...
impl GoosePromptSession {
    pub fn new(
        ...
        Self {
            ...
            output_tx,
            message_count: 0,
            session_name: format!("agentsmith-{}", session_id),
    ...
    pub async fn start(&mut self) -> Result<()> {
        tracing::info!("Goose prompt session ready (session name: {})", self.session_name);
        Ok(())
    }

    /// Send user input by spawning a `goose run` process.
    pub async fn send_input(&mut self, text: &str) -> Result<()> {
        {
            let st = self.state.lock().await;
            if *st == PromptState::Running {
                anyhow::bail!("Goose is still processing the previous message. Please wait.");
            }
        }

        // Build command arguments
        // First message: `goose run -n <name> -t "text"` (creates a new named session).
        // Subsequent messages: `goose run -n <name> -r -t "text"` (resumes the named session).
        let mut args = vec!["run".to_string()];
        args.extend(["-n".to_string(), self.session_name.clone()]);

        if self.message_count > 0 {
            args.push("-r".to_string());
        }

        args.extend(["-t".to_string(), text.to_string()]);
        args.extend(self.config.extra_args.clone());

        tracing::debug!(
            "Spawning goose prompt: {} {}",
            self.config.binary,
            args.join(" ")
        );
...
impl Drop for GoosePromptSession {
    fn drop(&mut self) {
