...
/// A Claude Code session running in an interactive PTY.
...
impl ClaudeSession {
    ...
    /// Start the Claude Code CLI in a PTY (interactive mode).
                ...
                tokio::select! {
                    raw = data_rx.recv() => {
                        match raw {
                            ...
                            None => {
                                tracing::debug!("Claude PTY output reader finished");
                                break;
                            }
                        }
                    }
    ...
    /// Send user input to the Claude CLI via PTY stdin.
    ...
    pub fn send_input(&mut self, text: &str) -> Result<()> {
        if let Some(ref mut pty) = self.pty {
            ...
            anyhow::bail!("Claude session not started");
        }
        Ok(())
    }
