...
impl<S: Store> Manager<S, Registered> {
    ...
    pub async fn send_message(
        ...
        // Sender certificate is needed for sealed sender (unidentified access).
        // If it fails (e.g. server changed certificate format), fall back to
        // identified sender which works fine but doesn't hide the sender.
        let sender_certificate = match self.sender_certificate().await {
            Ok(cert) => Some(cert),
            Err(e) => {
                warn!("failed to get sender certificate, falling back to identified sender: {e}");
                None
            }
        };
        let unidentified_access = match sender_certificate {
            Some(ref cert) => self
                .store
